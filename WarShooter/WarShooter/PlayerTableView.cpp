...
CPlayerTableView::CPlayerTableView(SAssets & assets)
	:m_assets(assets)
{
	m_name.setFont(m_assets.CRETE_ROUND_FONT);
	m_name.setCharacterSize(CHARACTER_SIZE);
	m_name.setPosition(DEFAULT_POSITION);

	m_status.setFont(m_assets.CRETE_ROUND_FONT);
	m_status.setCharacterSize(CHARACTER_SIZE);
	m_name.setPosition(DEFAULT_POSITION);

	m_killCount.setFont(m_assets.CRETE_ROUND_FONT);
	m_killCount.setCharacterSize(CHARACTER_SIZE);
	m_name.setPosition(DEFAULT_POSITION);

	m_deathCount.setFont(m_assets.CRETE_ROUND_FONT);
...
void CPlayerTableView::SetOpportunityDrawing(bool isDraw)
{
	m_isDraw = isDraw;
}
